"""
Triton implementation of causal_mask.

This implementation applies causal masking to a score tensor.

scores  : [B, Q, K]

returns : [B, Q, K]

Invalid future positions are masked to -inf.

This kernel is intentionally written to be readable and reusable as a
reference implementation of causal masking.
"""
...
def _causal_mask_kernel(
    ...
    causal_valid = offs_k[None, :] <= offs_q[:, None]
    out_vals = tl.where(causal_valid, vals, float("-inf"))
...
def causal_mask(scores: torch.Tensor):
    """
    Apply causal masking to a score tensor.
